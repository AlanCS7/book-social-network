...
import io.github.alancs7.book.file.FileStorageService;
...
public class BookService {
    ...
    public Long returnBorrowedBook(Long bookId, Authentication connectedUser) {
        ...
        var bookTransactionHistory = transactionHistoryRepository.findByBookIdAndUserId(bookId, user.getId())
    ...
    public Long approveReturnBorrowedBook(Long bookId, Authentication connectedUser) {
        ...
        User user = (User) connectedUser.getPrincipal();
        if (!Objects.equals(book.getOwner().getId(), user.getId())) {
            throw new OperationNotPermittedException("You cannot return a book that is not yours");
        }

        var bookTransactionHistory = transactionHistoryRepository.findByBookIdAndOwnerId(bookId, user.getId())
                .orElseThrow(() -> new OperationNotPermittedException("The requested book is not returned yet. It cannot be approved"));
